...
| `legacy-stdio` | C object requests `legacy_stdio_definitions.lib` with `#pragma comment(lib, ...)` | Check availability of a standard compatibility archive. |
...
| `overlay` | Generate a VFS overlay, change cwd, then move the fixture | Tests cwd-sensitive backing paths without Cargo, a real SDK, or third-party source. |
...
`headers-stdio.patch` is a small candidate derived from public 0.8.21 source. `crt-mode-745.patch` contains only the toolchain configuration portion of public PR #745; CRT selection is already tracked upstream in [#721](https://github.com/hermeticbuild/hermetic-llvm/issues/721).
